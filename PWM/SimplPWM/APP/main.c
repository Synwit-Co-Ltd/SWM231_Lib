...
int main(void)
{
	...
	GPIO_Init(GPIOB, PIN8, 1, 0, 0, 0);			//调试指示引脚
	
	PORT_Init(PORTA, PIN5, PORTA_PIN5_PWM0A,  0);
	PORT_Init(PORTA, PIN4, PORTA_PIN4_PWM0AN, 0);
	PORT_Init(PORTB, PIN6, PORTB_PIN6_PWM0B,  0);
//	PORT_Init(PORTB, PIN7, PORTB_PIN7_PWM0BN, 0);	// ACMP0_INN
	
	PORT_Init(PORTA, PIN7, PORTA_PIN7_PWM1A,  0);
	PORT_Init(PORTA, PIN6, PORTA_PIN6_PWM1AN, 0);
	PORT_Init(PORTA, PIN3, PORTA_PIN3_PWM1B,  0);
	PORT_Init(PORTA, PIN2, PORTA_PIN2_PWM1BN, 0);
...
#if 0
#if 1
//	PORT_Init(PORTA, PIN9, PORTA_PIN9_PWM_BRK0, 1);		// XTAL_IN
//	PORTA->PULLU |= (1 << PIN9);
	PORT_Init(PORTB, PIN2, PORTB_PIN2_PWM_BRK1, 1);
	PORTB->PULLU |= (1 << PIN2);
	
	PWM_BrkInPolarity(PWM_BRK1, 0);		// PWM_BRK1 低电平刹车
	
	PWM_BrkConfig(PWM0, PWM_CH_A, PWM_BRK1, 0, 1, 1, 0);	//PWM0通道A 受刹车输入 PWM_BRK1 控制
	PWM_BrkConfig(PWM0, PWM_CH_B, PWM_BRK1, 0, 1, 1, 0);	//PWM0通道B 受刹车输入 PWM_BRK1 控制
#else	// 比较器输出作为刹车信号
