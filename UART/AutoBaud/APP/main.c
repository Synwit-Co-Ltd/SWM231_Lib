...
int main(void)
{
	...
	/* 测试说明
	*  将UART1_TX脚连接到UART0_RX，UART1波特率57600，UART0波特率9600
	*  让UART1发送0xF8，UART0通过测量0xF8自动计算出波特率将自己的波特率设为57600
	****************************************************************************/
	UART_ABRStart(UART0, 0xF8);
	
	while((res = UART_ABRIsDone(UART0)) == 0)
	{
